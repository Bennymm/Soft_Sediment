...
mean_cover_seasonal <- function(Species, Title) {
  filter(ma.cover.y.m.s, species == Species, year == 2017, cover > 0) %>%
    ggplot() +
...
mean_cover_seasonal("lola", expression(italic(Lola~lubrica)))
...
cover_seasonal_box("ulva")

#total cover
#seasonal %cover; all species combined; standard error
filter(ma.cover.total,year == 2017) %>%
  ggplot() +
  ...
  theme(axis.text.x      = element_text(size = 10, angle = 45, hjust = 1),
        ...
        panel.grid.minor = element_blank(),
        text             = element_text(size = 13, vjust = 0.1)) +
  labs(x = "Month", y = expression("Mean Total Cover(%)"), colour = "site")

#cover all species combined
filter(ma.cover.absolute, year == 2017) %>%
  ggplot() +
